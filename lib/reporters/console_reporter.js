...
function ConsoleReporter(options) {
  ...
  this.jasmineDone = function() {
    ...
    if(specCount > 0) {
      ...
      var specCounts = executableSpecCount + ' ' + plural('spec', executableSpecCount) + ', ' +
        coloredIf('red', failureCount + ' ' + plural('failure', failureCount), failureCount > 0);
  ...
  function coloredIf(color, str, condition) {
    if(showColors)
    {
      return condition ? (ansi[color] + str + ansi.none) : str;
    }
    else
    {
      return str;
