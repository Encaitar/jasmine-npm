...
function ConsoleReporter(options) {
  ...
  this.jasmineDone = function() {
    ...
    if(specCount > 0) {
      ...
      var specCounts = executableSpecCount + ' ' + plural('spec', executableSpecCount) + ', ' +
        coloredIf('green', 'red', failureCount + ' ' + plural('failure', failureCount), failureCount > 0);
  ...
  function coloredIf(pass, fail, str, condition) {
    if(showColors)
    {
      return condition ? (ansi[fail] + str + ansi.none) : (ansi[pass] + str + ansi.none);
    }
    else
    {
      return str;
